...
module.exports = function(app){
	app.get('/u/:username/:taskNo', function(req,res){
			var no = req.params.taskNo,name = req.params.username;
			if (no && name){
	...
	app.post('/like_result', function(req,res){
			var no = req.body.taskNo,name = req.body.name;
			if (no && name){
				Result.get(name,no, function(err,result){
						if (err) return handleError(err,req,res);
						if (result){
	...
	app.get('/allResult/:taskNo', function(req,res){
			...
			Result.find({taskNo:taskNo,status:"done"})
	...
	app.get('/analyse', function(req,res){
			Task.find({},function(err,tasks){
					...
					tasks.forEach(function(task){
							if (task.no.length > 2){
											countTask+=2;
										if (countTask === tasks.length*2) res.end();
										return;
							}
							Result.find({taskNo:task.no})
							.count(function(err,count){
									res.write("task"+task.no+"/"+count+" accepted</br>");
									countTask++;
									if (countTask === tasks.length*2) res.end();
								});
							Result.find({taskNo:task.no,status:"done"})
							.count(function(err,count){
									res.write("task"+task.no+"/"+count+" done</br>");
									countTask++;
									if (countTask === tasks.length*2) res.end();
								});
						});
