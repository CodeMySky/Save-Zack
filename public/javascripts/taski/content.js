...
$(function () {
	...
	mc2.taski.hint = JSON.parse(unescape($("#tasks-data").html())).info.hint;
	if (response.status) {
	...
	if (!mc2.taski.files) {
		...
		if (parseInt(mc2.taski.task_seq) !== 10 && parseInt(mc2.taski.task_seq) !== 14 && parseInt(mc2.taski.task_seq) !== 15) {
			mc2.taski.files = JSON.parse(localStorage.getItem("files_task"+(mc2.taski.task_seq-1)));
		}
		if ( !mc2.taski.files) {
			//如果前一个任务也没有
			mc2.taski.files = {
